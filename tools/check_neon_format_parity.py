...
GROUPS = {
    "stem": (
        ROOT / "assets/stems/neon",
        ["drums", "bass", "chords", "melody", "sparkle"],
    ),
    "stinger": (
        ROOT / "assets/stingers/neon",
        ["victory", "gameover"],
    ),
    "transition": (
        ROOT / "assets/transitions/neon",
...
def main() -> None:
    ...
    report_path = ROOT / "qa/out/neon-format-parity.json"
    ...
    if errors:
        print("Neon Cross-Format Audio Parity Check FAILED")
        for error in errors:
            print(f"- {error}")
        raise SystemExit(1)

    print("Neon Cross-Format Audio Parity Check PASSED")
