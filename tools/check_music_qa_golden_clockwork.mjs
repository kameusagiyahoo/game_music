...
import {
  ...
  buildGoldenQaReport,
  writeGoldenQaReport,
} from "./music_qa_golden_clockwork.mjs";

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const baselinePath = path.join(ROOT, "qa", "baselines", "clockwork-standard-v1.json");
...
if (errors.length) {
  console.error("Clockwork Golden QA Gate Check FAILED");
  errors.forEach((error) => console.error("- " + error));
  process.exit(1);
}

console.log("Clockwork Golden QA Gate Check PASSED");
