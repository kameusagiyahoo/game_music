...
GROUPS = {
    "stem": (
        ROOT / "assets/stems/clockwork",
        ["drums", "bass", "chords", "melody", "sparkle"],
    ),
    "stinger": (
        ROOT / "assets/stingers/clockwork",
        ["victory", "gameover"],
    ),
    "transition": (
        ROOT / "assets/transitions/clockwork",
...
def main() -> None:
    ...
    report = {
        "schemaVersion": "1.0.0",
        "type": "clockwork-cross-format-parity",
        "referenceFormat": "wav",
    ...
    report_path = ROOT / "qa/out/clockwork-format-parity.json"
    ...
    if errors:
        print("Clockwork Cross-Format Audio Parity Check FAILED")
        for error in errors:
            print(f"- {error}")
        raise SystemExit(1)

    print("Clockwork Cross-Format Audio Parity Check PASSED")
