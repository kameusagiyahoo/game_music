...
const manager = new WavStemMusicManager({ pack: neonPack });
await manager.init();

const info = manager.getMasteringInfo();
const expectedTrim = 10 ** (-3 / 20);

if (info.profile !== "neon-drive-v1") {
...
if (!near(info.limiter.thresholdDb, -1.25)) {
...
if (!near(info.limiter.attack, 0.0025)) {
  errors.push(`limiter attack mismatch: ${info.limiter.attack}`);
}
if (!near(info.limiter.release, 0.10)) {
  errors.push(`limiter release mismatch: ${info.limiter.release}`);
}
if (manager.master.connections[0] !== manager.masterTrim) {
  errors.push("master is not connected to Neon headroom trim");
}
if (manager.masterTrim.connections[0] !== manager.limiter) {
  errors.push("Neon headroom trim is not connected to limiter");
}
if (!manager.limiter.connections.includes(manager.context.destination)) {
  errors.push("Neon limiter is not connected to destination");
}

if (errors.length) {
  console.error("Neon Runtime Mastering Check FAILED");
  errors.forEach((error) => console.error(`- ${error}`));
  process.exit(1);
}

console.log("Neon Runtime Mastering Check PASSED");
console.log("- profile: neon-drive-v1");
console.log("- headroom: -3.0 dB");
console.log("- limiter: -1.25 dB / 20:1");
console.log("- attack/release: 2.5 ms / 100 ms");
