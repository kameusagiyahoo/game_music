...
import { clockworkPack, clockworkManifest } from "../src/music-packs/clockwork.js";

const CLOCKWORK_QA_SCENARIO = Object.freeze({
  id: "clockwork-standard-v1",
  version: "1.0.0",
  durationMs: 60_000,
});

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const BASELINE_PATH = path.join(ROOT, "qa", "baselines", "clockwork-standard-v1.json");
...
function assetPath(kind, name) {
  if (kind === "stems") return "assets/stems/clockwork/" + name + ".wav";
  if (kind === "stingers") return "assets/stingers/clockwork/" + name + ".wav";
  if (kind === "transitions") return "assets/transitions/clockwork/" + name + ".wav";
...
function measureStage({ stage, config, assets, sampleRate, trimGain }) {
  const preset = clockworkPack.layerPresets?.[config.preset];
  if (!preset) throw new Error("Missing Clockwork layer preset: " + config.preset);
...
function fingerprintInputs(assets) {
  ...
  hash.update(JSON.stringify({
    layerPresets: clockworkPack.layerPresets,
    mastering: clockworkPack.mastering,
    scenario: CLOCKWORK_QA_SCENARIO,
...
export function buildGoldenCandidate() {
  ...
  const headroomDb = Number(clockworkPack.mastering?.headroomDb ?? -3);
  ...
  return {
    ...
    id: "clockwork-standard-v1-golden",
    renderProfile: RENDER_PROFILE,
    pack: {
      id: clockworkManifest.id,
      version: clockworkManifest.version,
      masteringProfile: clockworkManifest.masteringProfile,
      facadeApi: clockworkManifest.facadeApi,
    },
    scenario: {
      id: CLOCKWORK_QA_SCENARIO.id,
      version: CLOCKWORK_QA_SCENARIO.version,
      durationMs: CLOCKWORK_QA_SCENARIO.durationMs,
    },
    audio: {
      sampleRate,
      channels: 2,
...
export function buildGoldenQaMarkdown(
  ...
  lines.push(
    ...
    "",
    "_Generated from repository WAV files by tools/music_qa_golden_clockwork.mjs._",
    "",
  );
...
function main() {
  ...
  if (!fs.existsSync(BASELINE_PATH)) {
    ...
    console.error("Bootstrap with: node tools/music_qa_golden_clockwork.mjs --write");
    process.exit(1);
  }
  ...
  console.log("Clockwork Golden QA Regression Gate");
  ...
  if (!result.passed) {
    console.error("Clockwork Golden QA Regression Gate FAILED");
    result.failures.forEach((line) => {
      console.error("- " + line);
      if (process.env.GITHUB_ACTIONS === "true") {
        console.error("::error title=Clockwork Golden QA::" + githubCommandEscape(line));
      }
    });
    process.exit(1);
  }

  console.log("Clockwork Golden QA Regression Gate PASSED");
