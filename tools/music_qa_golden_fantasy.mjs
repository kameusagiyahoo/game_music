...
import { fantasyPack, fantasyManifest } from "../src/music-packs/fantasy.js";

const FANTASY_QA_SCENARIO = Object.freeze({
  id: "fantasy-standard-v1",
  version: "1.0.0",
  durationMs: 60_000,
});

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const BASELINE_PATH = path.join(ROOT, "qa", "baselines", "fantasy-standard-v1.json");
...
function assetPath(kind, name) {
  if (kind === "stems") return "assets/stems/fantasy/" + name + ".wav";
  if (kind === "stingers") return "assets/stingers/fantasy/" + name + ".wav";
  if (kind === "transitions") return "assets/transitions/fantasy/" + name + ".wav";
...
function measureStage({ stage, config, assets, sampleRate, trimGain }) {
  const preset = fantasyPack.layerPresets?.[config.preset];
  if (!preset) throw new Error("Missing Fantasy layer preset: " + config.preset);
...
function fingerprintInputs(assets) {
  ...
  hash.update(JSON.stringify({
    layerPresets: fantasyPack.layerPresets,
    mastering: fantasyPack.mastering,
    scenario: FANTASY_QA_SCENARIO,
...
export function buildGoldenCandidate() {
  ...
  const headroomDb = Number(fantasyPack.mastering?.headroomDb ?? -3);
  ...
  return {
    ...
    id: "fantasy-standard-v1-golden",
    renderProfile: RENDER_PROFILE,
    pack: {
      id: fantasyManifest.id,
      version: fantasyManifest.version,
      masteringProfile: fantasyManifest.masteringProfile,
      facadeApi: fantasyManifest.facadeApi,
    },
    scenario: {
      id: FANTASY_QA_SCENARIO.id,
      version: FANTASY_QA_SCENARIO.version,
      durationMs: FANTASY_QA_SCENARIO.durationMs,
    },
    audio: {
      sampleRate,
      channels: 2,
...
export function buildGoldenQaMarkdown(
  ...
  lines.push(
    ...
    "",
    "_Generated from repository WAV files by tools/music_qa_golden_fantasy.mjs._",
    "",
  );
...
function main() {
  ...
  if (!fs.existsSync(BASELINE_PATH)) {
    ...
    console.error("Bootstrap with: node tools/music_qa_golden_fantasy.mjs --write");
    process.exit(1);
  }
  ...
  console.log("Fantasy Golden QA Regression Gate");
  ...
  if (!result.passed) {
    console.error("Fantasy Golden QA Regression Gate FAILED");
    result.failures.forEach((line) => {
      console.error("- " + line);
      if (process.env.GITHUB_ACTIONS === "true") {
        console.error("::error title=Fantasy Golden QA::" + githubCommandEscape(line));
      }
    });
    process.exit(1);
  }

  console.log("Fantasy Golden QA Regression Gate PASSED");
