...
import { neonPack, neonManifest } from "../src/music-packs/neon.js";

const NEON_QA_SCENARIO = Object.freeze({
  id: "neon-standard-v1",
  version: "1.0.0",
  durationMs: 60_000,
});
...
const BASELINE_PATH = path.join(ROOT, "qa", "baselines", "neon-standard-v1.json");
...
function assetPath(kind, name) {
  if (kind === "stems") return "assets/stems/neon/" + name + ".wav";
  if (kind === "stingers") return "assets/stingers/neon/" + name + ".wav";
  if (kind === "transitions") return "assets/transitions/neon/" + name + ".wav";
...
function measureStage({ stage, config, assets, sampleRate, trimGain }) {
  const preset = neonPack.layerPresets?.[config.preset];
  if (!preset) throw new Error("Missing Neon layer preset: " + config.preset);
...
function fingerprintInputs(assets) {
  ...
  hash.update(JSON.stringify({
    layerPresets: neonPack.layerPresets,
    mastering: neonPack.mastering,
    scenario: NEON_QA_SCENARIO,
...
export function buildGoldenCandidate() {
  ...
  const headroomDb = Number(neonPack.mastering?.headroomDb ?? -3);
  ...
  return {
    ...
    id: "neon-standard-v1-golden",
    renderProfile: RENDER_PROFILE,
    pack: {
      id: neonManifest.id,
      version: neonManifest.version,
      masteringProfile: neonManifest.masteringProfile,
      facadeApi: neonManifest.facadeApi,
    },
    scenario: {
      id: NEON_QA_SCENARIO.id,
      version: NEON_QA_SCENARIO.version,
      durationMs: NEON_QA_SCENARIO.durationMs,
    },
    audio: {
      sampleRate,
      channels: 2,
...
export function buildGoldenQaMarkdown(
  ...
  lines.push(
    ...
    "",
    "_Generated from repository WAV files by tools/music_qa_golden_neon.mjs._",
    "",
  );
...
function main() {
  ...
  if (!fs.existsSync(BASELINE_PATH)) {
    ...
    console.error("Bootstrap with: node tools/music_qa_golden_neon.mjs --write");
    process.exit(1);
  }
  ...
  console.log("Neon Golden QA Regression Gate");
  ...
  if (!result.passed) {
    console.error("Neon Golden QA Regression Gate FAILED");
    result.failures.forEach((line) => {
      console.error("- " + line);
      if (process.env.GITHUB_ACTIONS === "true") {
        console.error("::error title=Neon Golden QA::" + githubCommandEscape(line));
      }
    });
    process.exit(1);
  }

  console.log("Neon Golden QA Regression Gate PASSED");
