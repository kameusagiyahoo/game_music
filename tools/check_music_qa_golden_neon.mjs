...
import {
  ...
  buildGoldenQaReport,
  writeGoldenQaReport,
} from "./music_qa_golden_neon.mjs";

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const baselinePath = path.join(ROOT, "qa", "baselines", "neon-standard-v1.json");
...
if (errors.length) {
  console.error("Neon Golden QA Gate Check FAILED");
  errors.forEach((error) => console.error("- " + error));
  process.exit(1);
}

console.log("Neon Golden QA Gate Check PASSED");
